...
### Code reproduces Fig. 1 in LaManna et al. 2000 Ecosphere - Conspecific negative density dependence and why its study should not be abandonded
### Code modified from Detto et al. 2019, data from Detto et al. 2019
### by: J. LaManna, updated: 10-9-2020
...
for(s in 1:S) {
    ...
    if(N[s] > tr) {
        x0 = gx[use]; y0 = gy[use]
        D = dbh[use]
        if(type == 'random') {D = D[sample(length(D))]}		# Implements the relabeling null model advocated by Detto et al. 2019 -- ****THIS MODEL PRODUCES FALSE DENSITY DEPENDENCE AND SHOULD NOT BE USED FOR SPATIAL CNDD ANALYSES, as shown in Fig. 2
        D50 = quantile(D, quant)	# select adult/sapling cutoff point - this is where one could set a custom DBH cutoff point for each species (recoding would be required, seting a new D50 value for each species in the analysis, individuals below D50 are classified as saplings, and individuals above D50 are classified as adults)
...
null.its.k = 100			# Iterations for each batch of null model iterations (final iterations will be this number times null.k, which is the number of null-model batches to run)
null.k = 10				# Number of batches of null model iterations to run (final iterations will be this number times null.its.k, which is the number of iterations in each batch)

# Prepare BCI data
sp = bci$sp
bci$dbh = bci$dbh * 0.1			# BCI provides DBH in mm, not cm. This coverts DBH to cm
dbh = bci$dbh
gx = bci$gx
gy = bci$gy
species = unique(bci$sp)
S = length(species)
Lx = 1000; Ly = 500; bci.Area = 50	# Lx = East-west length of plot; Ly = North-south length of plot; bci.Area is in ha
tr=25					# tr = minimum abundance for species to be included in the plot -- this is code modified from Detto et al. 2019, who selected 0.5 individuals/ha as the inclusion criterion
...
# Relabel null model   ****THIS MODEL PRODUCES FALSE DENSITY DEPENDENCE AND SHOULD NOT BE USED FOR SPATIAL CNDD ANALYSES, as shown in Fig. 2
null.type = 'random'
set.seed(892)
bci.slr = rep(NA, times = S)
...
# Prepare SERC data
sp = serc$sp
serc$dbh = serc$dbh * 0.1			# SERC provides DBH in mm, not cm. This coverts DBH to cm
dbh = serc$dbh
gx = serc$gx
gy = serc$gy
